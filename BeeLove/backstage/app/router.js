module.exports = app => {
	...
	router.post("/adminlogin", controller.loginController.adminlogin); // 管理员登录
	
	router.get("/feedback", controller.feedbackController.feedback) //获取反馈数据
	router.get("/delFeedId", controller.feedbackController.delFeedId) //删除指定id的反馈
	router.get("/delChecked", controller.feedbackController.delChecked) //删除选择的反馈
	
	router.get("/getAllUser",controller.adminController.getAllUser);//得到所有用户
	router.post("/lockUser",controller.adminController.lockUser);//用户封号
	router.post("/unlockUser",controller.adminController.unlockUser);//用户解封
	...
	router.get("/getava", controller.personalcenterController.getava);
