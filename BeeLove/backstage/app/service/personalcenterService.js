...
class personalcenterService extends Service {
	...
	async getava() {
		...
		let sql = "select avatar from user where username=?";
		let res = await this.ctx.app.mysql.query(sql, [username]);
		if (res.length == 0) {
			return "http://localhost:9999/public/userAvatar/userdefaultavatar.png";
		} else {
			return res[0].avatar;
		}
	}
	...
	async aplmyfriend() {
		...
		if (res1.length == 0) {
			...
			if (res2.length == 0) {
				...
				let res3 = await this.ctx.app.mysql.query(sql3, [person_id, user_id]);
